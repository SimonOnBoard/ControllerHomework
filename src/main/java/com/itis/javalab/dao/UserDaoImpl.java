...
public class UserDaoImpl implements UserDao {
    ...
    private UserRowMapper<User> userFindRowMapper = (row, dto) -> {
        ...
        String photoPath = row.getString("photopath");
        Date birthday = row.getObject("birthday", Date.class);
        LocalDateTime registration = row.getObject("registration_date", LocalDateTime.class);

        return new User(id, nick, mail, birthday, registration, photoPath, role, dto);
    };
    @Autowired
    ...
    //language=sql
    private static final String SQL_INSERT = "INSERT INTO users_servlet (nickname, email, birthday, registration_date, photopath, role, authdata_id) " +
            "VALUES (?,?,?,?,?,?,?)";
    ...
    public void save(User model) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            ...
            statement.setObject(3, model.getBirthDate());
            statement.setObject(4, model.getRegiStrationDate());
            statement.setString(5, model.getPhotoPath());
            statement.setString(6, model.getRole());
            statement.setLong(7, model.getAuthDataDTO().getId());
    ...
    public Optional<User> findByDTO(AuthDataDTO dto) {
        User user = null;
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM users_servlet WHERE id = ?")) {
